...
module.exports = {
  apps: [
    ...
    },
    {
      ...
      args: [
        "--noprofile", "--norc", "-c",
        `${LOAD_ENV}; ${LIMITLESS_ENV} "${PYTHON}" -m limitless_hl.daemon --intervals 1m,5m,15m,1h,1d,1w --sides UP,DOWN --scoring-live --slice-score-file tmp/limitless_hl/evaluation_report_live.json --slice-min-n 0 --slice-min-roi -1.0 --slice-min-win-rate 0.0 --score-min 0.0 --score-base-stake-usdc 1 --score-max-stake-usdc 1 --hl-bot-status-file /opt/hyperliquid-bot/hl_bot_status.json --min-edge 0.0 --max-price 0.95 --min-seconds-to-expiry 45 --stake-usdc 1 --max-daily-loss-usdc 999 --max-open-markets 100 --loop-seconds 15 --book-log tmp/limitless_hl/book_snapshots.jsonl --jsonl-out tmp/limitless_hl/daemon_shadow.jsonl`,
      ],
      autorestart: true,
      restart_delay: 5000,
      max_restarts: 50,
